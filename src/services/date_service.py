...
# Portuguese month names (used in Excel templates and string month input)
_PT_MONTH_NAMES = {
    "Janeiro": 1,
    "Fevereiro": 2,
    "Março": 3,
...
class DateService:
    ...
    def resolve_month(month: int | str) -> int:
        ...
        if isinstance(month, str):
            num = _PT_MONTH_NAMES.get(month)
            if num is None:
                msg = f"Unknown month name: {month}"
                raise ValueError(msg)
            return num
        msg = "month must be int (1–12) or a Portuguese month name"
        raise TypeError(msg)

    @staticmethod
    def month_name_portuguese(month: int) -> str:
        """Portuguese month label for Excel templates (1 = Janeiro, …, 12 = Dezembro)."""
